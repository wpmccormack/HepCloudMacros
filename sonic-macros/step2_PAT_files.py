...
for line in linesOfFiles:
    print('am I in file?')
    print(line)
    print(cwd+'/'+line.split("/")[-1][:-1])
    if(path.exists(cwd+'/'+line.split("/")[-1][:-1])):
        inFiles.append('file:'+cwd+'/'+line.split("/")[-1][:-1])
        print('appending file:'+cwd+'/'+line.split("/")[-1][:-1])
...
# Input source
filesVString = cms.untracked.vstring()
for f in inFiles:
    filesVString.append(f)

process.source = cms.Source("PoolSource",
    fileNames = filesVString,

...
cd ..
xrdcp -s root://cmseos.fnal.gov//store/user/wmccorma/CMSSW_12_0_0_pre5_verbose.tgz ./CMSSW_12_0_0_pre5.tgz
source /cvmfs/cms.cern.ch/cmsset_default.sh
...
rm run.py
xrdcp -s root://cmseos.fnal.gov//store/user/wmccorma/run_nofallback.py ./run.py
...
cmsRun run.py sonic=False tmi=True threads=4 maxEvents=1
sleep 60
cmsRun run.py sonic=False tmi=True threads=4 maxEvents=200 verbose=True
#cmsRun run.py maxEvents=100 sonic=False tmi=True threads=4
